...
void IdentifyVersion (void) {
  ...
  if(!_access(doom2wad, 04)) {
    gamemode = commercial;
    D_AddFile(doom2wad);
    return;
  }

  if(!_access(plutoniawad, 04)) {
    gamemode = commercial;
    D_AddFile(plutoniawad);
    return;
  }

  if(!_access(tntwad, 04)) {
    gamemode = commercial;
    D_AddFile(tntwad);
    return;
  }

  if(!_access(doomuwad, 04)) {
    gamemode = retail;
    D_AddFile(doomuwad);
    return;
  }

  if(!_access(doomwad, 04)) {
    gamemode = registered;
    D_AddFile(doomwad);
    return;
  }

  if(!_access(doom1wad, 04)) {
    gamemode = shareware;
    D_AddFile(doom1wad);
    return;
  }
...
void FindResponseFile(void) {
  ...
  for(int i = 1; i < myargc; i++) {
    ...
    char *infile = file;
    int indexinfile = 1; // SKIP PAST ARGV[0] (KEEP IT)
    k = 0;
    do {
